...
class FuelGrid: 
    ...
    fuel_density_threshold: int = 1 # minimum fuels per grid cell to be considered as part of a larger cluster
    image_width: int = 640
    ...
    def check_fuel_density(self, cluster: fuelcluster.FuelCluster):
        return cluster.fuel_count >= FuelGrid.fuel_density_threshold
    
    def get_unique_clusters(self, width, height):
        unique_clusters: list[fuelcluster.FuelCluster] = []
        for w in range(width + 1):
            for h in range(height + 1):
                if(unique_clusters.count(self.cluster_grid[w][h]) == 0):
                    print(self.cluster_grid[w][h].avg_x)
                    unique_clusters.append(self.cluster_grid[w][h])
    ...
    def find_clusters(self):
        ...
        for w in range(width + 1): #Create clusters for each grid cell
            for h in range(height + 1):
        ...
        for w in range(width + 1): #Combine grid cell clusters into one whole
            for h in range(height + 1):
                cluster: fuelcluster.FuelCluster = self.cluster_grid[w][h]
                if (self.check_fuel_density(cluster)):
                    if ((w + 1 < width) and not(self.cluster_grid[w + 1][h] is None) and self.check_fuel_density(self.cluster_grid[w + 1][h])):
                        cluster.merge_clusters(self.cluster_grid[w + 1][h])
                        self.cluster_grid[w + 1][h] = cluster
                    if ((w - 1 >= 0) and not(self.cluster_grid[w - 1][h] is None) and self.check_fuel_density(self.cluster_grid[w - 1][h])):
                        cluster.merge_clusters(self.cluster_grid[w - 1][h])
                        self.cluster_grid[w - 1][h] = cluster
                    if ((h + 1 < height) and not(self.cluster_grid[w][h + 1] is None) and self.check_fuel_density(self.cluster_grid[w][h + 1])):
                        cluster.merge_clusters(self.cluster_grid[w][h + 1])
                        self.cluster_grid[w][h + 1] = cluster
                    if ((h - 1 < height) and not(self.cluster_grid[w][h - 1] is None) and self.check_fuel_density(self.cluster_grid[w][h - 1])):
                        cluster.merge_clusters(self.cluster_grid[w][h - 1])
                        self.cluster_grid[w][h - 1] = cluster

        clusters = self.get_unique_clusters(width, height)
        return clusters
...
if True: # debug flag, mostly for testing on single images to get less confusing and repetitive logs.
    ...
    time.sleep(2) # Necessary for debuging or running on a single frame to make sure NT data is present and ready to pull.
    values = fuelValues.get()
    grid.split_fuel_string(values)
    #print(grid.total_fuel)
    print("finding clusters")
